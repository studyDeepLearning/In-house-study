...
詳しいラムダ式の記法についてはおいといて、とりあえず、↑の例をラムダ式を用いて書き換えてみます。
...
なんか短くなりましたね。  
やっていることは匿名内部クラスの場合と同じで、足し算を行うメソッド add を実装して、クラスをインスタンス化しているのです。

ですが、通常インスタンス化に必要な new 演算子やメソッドのオーバーライドの記述が省略されているため、非常に簡潔になっていて、足し算を行う**関数（メソッド）をメソッドの引数に直接渡している**ように見えます。
